...
class IsomorphicSuperimposer(StructureSuperimposer):
    ...
    def _custom_graph_match(self, ligand_graph: nx.Graph, cyclic_peptide_graph: nx.Graph) -> Dict[Any, Any]:
        ...
        def node_matches(node1, node2):
            return (ligand_graph.nodes[node1]['element'] == cyclic_peptide_graph.nodes[node2]['element'])

        def explore_neighborhood(ligand_node, cyclic_node, visited_ligand=None, visited_cyclic=None):
            ...
            for lig_neighbor in ligand_neighbors:
                ...
                potential_matches = [
                    cyc_neighbor for cyc_neighbor in cyclic_neighbors
...
class StructureAligner:
        ...
        with open(filepath, 'r') as f:
            for line in f:
                if line.startswith('MODEL'):
                    ...
                    if reading_model:
                        break
                elif reading_model and (line.startswith('ATOM') or line.startswith('HETATM')):
                    atoms.append(line)
    ...
    def detect_clashes(
        ...
        for chain_id in target_chains:
            if chain_id in structure[0]:
                target_atoms.extend(list(unfold_entities(
                    structure[0][chain_id], 'A')))
    ...
    def align_structures(
        ...
        try:
            ...
            # Extract atoms
            cyclic_atoms = list(unfold_entities(
                cyclic_struct[0][cyclic_chain], 'A'))
            ligand_atoms = list(unfold_entities(
                ligand_struct[0][ligand_chain], 'A'))
            ...
            if result.transformation_matrix is not None:
                ...
                if write_pdbs and output_dir:
                    ...
                    output_path = os.path.join(output_dir, f"aligned_conf_{model_number+1}.pdb")
...
def process_single_model(args):
    ...
    (pdb_path, model_idx, reference_pdb, ligand_chain,
     target_chains, clash_cutoff, write_pdbs, output_dir) = args
    ...
    try:
        ...
        conformer_output_dir = os.path.join(output_dir, os.path.splitext(os.path.basename(pdb_path))[0]) if write_pdbs else None
        ...
        if result.clash_results:
            clash_info.update({
                ...
                'Min_Distance': result.clash_results.min_distance
            })
...
def process_directory(
    ...
    for pdb_file in os.listdir(input_dir):
        ...
        for model_idx in range(model_count):
            processing_tasks.append((
                pdb_path, model_idx, reference_pdb, ligand_chain,
                target_chains, clash_cutoff, write_pdbs, output_dir
            ))
    ...
    # Process summary for each file
        ...
        for filename, file_results in results_by_file.items():
            ...
            compound_id = conformer_df['Compound'].dropna().iloc[0] if not conformer_df['Compound'].isna().all() else filename
            sequence = conformer_df['Sequence'].dropna().iloc[0] if not conformer_df['Sequence'].isna().all() else None
            ...
            if len(non_clashing_df) > 0:
                best_non_clashing_row = non_clashing_df.loc[non_clashing_df['RMSD'].idxmin()]
            ...
            summary_stats = {
                ...
                **best_non_clashing_info,
                ...
                'Min_Distance_Overall': conformer_df['Min_Distance'].min(),
            ...
            print(f"Best RMSD: {summary_stats['Min_RMSD']:.4f} (Model {summary_stats['Best_RMSD_Model']}, Frame {summary_stats['Best_RMSD_Frame']})")
            if summary_stats['Best_Non_Clashing_Model'] is not None:
                print(f"Best non-clashing RMSD: {summary_stats['Best_Non_Clashing_RMSD']:.4f} "
            ...
            print(f"Number of clashing conformers: {summary_stats['Num_Clashing_Conformers']}")
            print(f"Clash statistics:")
            print(f"  Range: {summary_stats['Min_Clashes']} - {summary_stats['Max_Clashes']} clashes")
            ...
            print(f"Minimum distance to target: {summary_stats['Min_Distance_Overall']:.2f} Å")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Align molecular structures and detect clashes.')
    ...
    parser.add_argument('--target_chains', nargs='+', help='Chain IDs to check for clashes against')
    parser.add_argument('--clash_cutoff', type=float, default=2.0, help='Distance cutoff for clash detection (Angstroms)')
    parser.add_argument('--write_pdbs', action='store_true', help='Write aligned PDB files')
